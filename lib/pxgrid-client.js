...
class PxgridRestClient {
  ...
  publishEndpointAssetUpdate(stompClient, assetBody) {
    let binaryData;
    if (assetBody) {
      const data = { opType: "UPDATE", asset: assetBody }
      binaryData = new TextEncoder().encode(JSON.stringify(data));
    } else throw new Error('No body provided to publish.');

    const receiptId = Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 12);
    ...
    if (this.endpointAssetService) {
      console.log('USING EXISTING SERVICE INFO');
      stompClient.publish({
      ...
      this._getServiceWithSecret(ENDPOINT_ASSET_SERVICE)
        .then(service => {
          console.log('USING NEW SERVICE INFO');
          console.log(service);
