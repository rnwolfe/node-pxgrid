...
class Control {
   ...
   * @memberof Control
   */
  activate(accountDesc = 'pxgrid-node', retryInterval = 60000, maxRetries = 10, retryAttempt = 1) {
      ...
      .then(response => {
        ...
        if (state === 'ENABLED') {
          ...
          if (retryAttempt > maxRetries) {
            throw new Error('Account state is PENDING (likely requires approval or has been disabled). Hit max number of retries (' + maxRetries + ').')
          } else {
            console.log('Account state is PENDING. Retrying in ' + retryInterval / 1000 + ' seconds (attempt: ' + retryAttempt + '). The account may need to be approved on the pxGrid controller.')
            return this._delay(60000)
              .then(() => this.activate(accountDesc));
          }
        } else if(state === 'DISABLED') {
          throw new Error('Client failed to activate because the account state is DISABLED! Please enable the account on the pxGrid controller and try again.');
        }
      })
   ...
   * @param {number} [maxRetries=10] Maximum retries that will be attempted.
   * @param {number} [retryAttempt=1] Which attempt we are on. This is necessary since we use recursion for retries.
   * @returns {Promise} Returns a list of nodes providing the specified service, as well as their properties. Empty if no publishers registered.
   */
  serviceLookup(serviceName, retryInterval=30000, maxRetries=10, retryAttempt=1) {
    ...
    return this._post(SERVICE_LOOKUP_URL, payload)
      .then(response => {
        if (!response.services[0]) {
          ...
          if (retryAttempt > maxRetries) {
            throw new Error('No registered publisher(s) for service/topic. Hit max number of retries (' + maxRetries + ').')
          } else {
            console.log('No publishing nodes registered for service ' + serviceName + ', retrying in ' + retryInterval / 1000 + ' seconds (attempt: ' + retryAttempt + ')...')
            return this._delay(retryInterval)
              .then(() => this.serviceLookup(serviceName, retryInterval, maxRetries, retryAttempt + 1));
          }
        } else {
