...
class Control {
  constructor({
    ...
    if (Array.isArray(this.config.hosts) && typeof this.config.hosts !== 'undefined') {
    ...
    if (typeof this.config.hostname === 'string' && this.hosts.length == 0) this.hosts[0] = this.config.hostname;

    this.config.verifySSL = typeof this.config.verifySSL === 'undefined' ? true : this.config.verifySSL;
    this.httpOptions = {
  ...
  async _post(url, body, debug = false) {
    for (let i = 0; i < this.hosts.length; i++) {
      const baseUrl = `https://${this.hosts[i]}:${
        this.config.port
        }/pxgrid/control`;
      ...
      try {
        ...
        if (debug) {
          console.debug(`Connection to ${host} failed, trying next..`)
        }
        continue;
      }
    }
